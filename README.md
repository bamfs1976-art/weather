...
| **Rivers & Sea** | Live river/sea gauge levels against their typical range, flood warnings and alerts, and sea state (wave height, period, swell, sea temperature) |
...
│       ├── water/          # River levels, flood warnings, sea state
...
└── lib/
    ...
    ├── water.ts            # SERVER ONLY — Environment Agency + Open-Meteo
...
- The Environment Agency and Open-Meteo both report in UTC. Times from those
  feeds are re-stamped with the selected place's UTC offset before display, so
  a reading taken at 18:44 BST does not show as 17:44.
